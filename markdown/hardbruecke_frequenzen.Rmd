...
output:
  flexdashboard::flex_dashboard:
    ...
    navbar:
      - { title: "Data", 
      href: "https://data.stadt-zuerich.ch/dataset/vbz_frequenzen_hardbruecke", 
      align: right, icon: "fa-solid fa-database"
      }
    theme: 
      version: 4
      bootswatch: flatly
...
```{r libraries and folder settings, echo=FALSE}
library(tidyr)
library(dplyr)
library(purrr)
library(readr)
...
```{r data reading from files, eval=TRUE, echo=FALSE}
# read data -------------
years <- 2020:2024
...
``` {r data reading from rds, eval=FALSE, echo=FALSE}
# load data faster from rds file
freq_df <- readRDS(file.path(data_folder, "freq_df.rds"))
```

``` {r data and colour settings, echo=FALSE}
...
### Absolute Werte von Ein- und Ausstiegen VBZ Fahrzeuge Ost {data-width=200}

- Dienstag, Mittwoch und Donnerstag sind die frequenzstärksten Tage

- Die absoluten Zahlen steigen an:

  - `r calc_increase(freq_vbz_east, 2020, 2023)` % im Jahr 2023 gegenüber 2020
    
  - `r calc_increase(freq_vbz_east, 2020, 2024)` % im Jahr 2024 gegenüber 2020 (Daten nur bis `r last_entry_date`)

- Alle Wochentage verzeichnen einen Anstieg

- Die Zahlen von Richtung "West" sind fehlerhaft seit 2023, daher wurde die Richtung "West" komplett ausgeschlossen

- **Steigen die Zahlen an jedem Wochentag gleich?**
...
### Relative Werte von Ein- und Ausstiegen VBZ Fahrzeuge Ost {data-width=200}

- Montag und Freitag wird relativ zu den anderen Wochentagen immer weniger genutzt. Möglicherweise Trends in:
  
  - Home-Office-Tagen
  
  - Teilzeitarbeit

- Die Daten von 2024 sind nur bis  `r last_entry_date` vorhanden: Achtung typische Feiertage im April und Mai

- Die Zahlen von Richtung "West" sind fehlerhaft seit 2023, daher wurde die Richtung "West" komplett ausgeschlossen

- **Werden diese Ergebnisse von allen Messorten bestätigt?**
...
### Absolute Werte von allen Messorten Ost {data-width=200}

- Dienstag, Mittwoch und Donnerstag sind die frequenzstärksten Tage

- Die absoluten Zahlen steigen an:

  - `r calc_increase(freq_all_loc_east, 2020, 2023)` % im Jahr 2023 gegenüber 2020
    
  - `r calc_increase(freq_all_loc_east, 2020, 2024)` % im Jahr 2024 gegenüber 2020 (Daten nur bis `r last_entry_date`)
...
### Relative Werte von allen Messorten Ost {data-width=200}
...
### Absolute Werte von allen Daten! {data-width=200}

- Dienstag, Mittwoch und Donnerstag sind die frequenzstärksten Tage

- Die absoluten Zahlen steigen an:

  - `r calc_increase(freq_all_loc, 2020, 2023)` % im Jahr 2023 gegenüber 2020
    
  - `r calc_increase(freq_all_loc, 2020, 2024)` % im Jahr 2024 gegenüber 2020 (Daten nur bis `r last_entry_date`)

- Alle Wochentage verzeichnen einen Anstieg

- Die Zahlen von Richtung "West" sind fehlerhaft seit 2023 und werden hier nur testweise gezeigt
...
### Relative Werte von allen Daten! {data-width=200}
